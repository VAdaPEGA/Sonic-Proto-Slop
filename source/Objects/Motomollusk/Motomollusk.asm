...
; ===========================================================================
		IndexStart
	GenerateIndex	 Obj54, Init
	GenerateIndex	 Obj54, Move
	GenerateIndex	 loc_177B4
	GenerateIndex	 Obj54, Face
	GenerateIndex	 Obj54, Thruster
; ===========================================================================
...
; loc_17688:
Obj54_Move:
	bsr.w	sub_176D0	; literally everything
	jsr	ObjectMove
	jsr	(ObjHitFloor).l	; check for floor
	cmpi.w	#-8,d1
	blt.s	@MakeMolluskFall	
		cmpi.w	#$C,d1
		bge.s	@MakeMolluskFall
			add.w	d1,y_pos(a0)	; snap to floor
			bra.s	Obj54_Display_2
	@MakeMolluskFall:
		addq.b	#2,routine(a0)
		move.w	#$14,Obj54_FallTimer(a0)
		st	$34(a0)
; ---------------------------------------------------------------------------
Obj54_Display_2:
		lea	(Ani_Obj54).l,a1
...
Obj54_Face:	
		movea.l	$2A(a0),a1
		cmpi.b	#ObjId_Motomollusk,(a1)	; is daddy still around?
		bne	DeleteObject		; if not, perish
			move.w	x_pos(a1),x_pos(a0)
			move.w	y_pos(a1),y_pos(a0)
